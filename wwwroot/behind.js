$(function () {
    ...
    var leaderArray = [
        { min: 'Martin', 'name': 'Martin'},
        { min: 'Forest', 'name': 'Forest'},
        { min: 'Xga', 'name': 'Xga'},
        { min: 'Vincent', 'name': 'Vincent'},
        { min: 'Ray', 'name': 'Ray'},
        { min: 'Ricky', 'name': 'Ricky'},
        { min: 'Royal', 'name': 'Royal'},
        { min: 'Philip', 'name': 'Philip'},
        { min: 'Bill', 'name': 'Bill'},
        { min: 'Howe', 'name': 'Howe'},
        { min: 'Thomas', 'name': 'Thomas'},
        { min: 'Daniel', 'name': 'Daniel'},
    ...
    let Page = {
        Init: function () {
            Page.Connection = new WebSocket(`ws://${window.location.host}/ws`);
            ...
            $("#btn-show-bag").bind("click", function () {
                Page.ShowBag();
            });
            $("#btn-hide-bag").bind("click", function () {
                Page.HideBage();
            });
        ...
        ShowBag: function () {
            Page.ViewOjb.PoolMoney = Page.GetPoolMoney();
            Page.ViewOjb.Type = "show_bag";
            Page.Connection.send(JSON.stringify(Page.ViewOjb));
        },
        HideBage: function () {
            Page.ViewOjb.PoolMoney = Page.GetPoolMoney();
            Page.ViewOjb.Type = "hide_bag";
            Page.Connection.send(JSON.stringify(Page.ViewOjb));
        },
        ViewEmpty: function () {
            Page.ViewOjb.PoolMoney = Page.GetPoolMoney();
        ...
        NSetDefaultValue: function () {
            if (parseInt($("#ndraw-leader-count").val()) >= -1) {
            } else {
                $("#ndraw-leader-count").val(-1);
            }
        },
        NGetDrawParam: function () {
            ...
            var param = {
                LeaderCount: leaderCount_value >=-1 ? leaderCount_value : "-1",
        ...
        NValidateDrawParam: function (thisParam) {
            ...
            var validateInfo = "";
            if (thisParam) {
                if (parseInt(thisParam.LeaderCount) > -2) {
                } else {
                    validateInfo += " 领导小于-1";
                    result = false;
                }
        ...
        NDrawReady: function () {
            ...
            try {
                ...
                if (nextAward.code == 0) {
                	Page.ReadyObj={}
					Page.ReadyObj.Type="ready"
					Page.ReadyObj.Drawer=""
        ...
        NGetAndSetNextAward: function () {
            $.ajax({
                ...
                success: function (info) {
                    ...
                    if (info.code == 0) {
                        $("#now-award-info").text("当前步骤:" + info.data.ID + "--奖品:【" + info.data.AwardID + "】" + info.data.AwardName + "--中奖总人数:" + info.data.PeopleCount + "--返奖金额:" + info.data.BackMoney);
                        $("#ndraw-leader-count").val(info.data.LeaderCount);
                    } else if (info.code == 1) {
        ...
        PoolGetDrawParam: function () {
            return {
                ...
                BackMoney: $("#pooldraw-leader-backmoney").val(),
